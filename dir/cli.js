...
// src/dictionaries/adjectives.json
...
var getRawProjName = function(options) {
  const raw = [];
  times(options.words - 1, function() {
    if (options.alliterative && raw.length) {
      const adjectiveSample = sample(getAlliterativeMatches(adjectives_default, raw[0].toString().substring(0, 1)));
      ...
      if (options.firstLetter) {
        adjectiveSample = sample(filter(adjectives_default, function(elm) {
          return elm.substring(0, 1).toLowerCase() === options.firstLetter;
        }));
      } else {
        adjectiveSample = sample(adjectives_default);
      }
      if (adjectiveSample !== undefined) {
  ...
  if (options.alliterative) {
    const nounSample = sample(getAlliterativeMatches(nouns_default, raw[0].toString().substring(0, 1)));
    if (nounSample !== undefined) {
      raw.push(nounSample);
    }
  } else {
    const noun = sample(nouns_default);
    if (noun !== undefined) {
...
var getAlliterativeMatches = function(arr, letter) {
...
if (args.includes("-h") || args.includes("--help")) {
  ...
  const srvName = generator_default(options);
