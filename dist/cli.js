...
var getRawProjName = function(options) {
  const raw = [];
  const adj = blocklistFilter(options.blocklist || [], adjectives_default);
  const nou = blocklistFilter(options.blocklist || [], nouns_default);
  times(options.words - 1, function() {
    if (options.alliterative && raw.length) {
      ...
      if (options.firstLetter && raw.length === 0) {
        adjectiveSample = sample(filter(adj, function(elm) {
...
function blocklistFilter(blocklist, raw) {
  return filter(raw, function(elm) {
    return blocklist.indexOf(elm.toString()) === -1;
  });
}
